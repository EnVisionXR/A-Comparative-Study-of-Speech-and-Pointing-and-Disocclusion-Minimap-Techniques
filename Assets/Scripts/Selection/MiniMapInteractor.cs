...
{
    List<Interactable> originalInteractables;
    private SearchExperimentTrial searchExperimentTrial;
    public Dictionary<GameObject, GameObject> originalToDuplicateMap = new Dictionary<GameObject, GameObject>();
    public Dictionary<GameObject, GameObject> originalToDuplicateCir = new Dictionary<GameObject, GameObject>();
    ...
    public Dictionary<Interactable, shapeItem_2> originalToDuplicate;
    ...
    private void Start()
    {
        searchExperimentLevel = UnityEngine.Object.FindObjectOfType<SearchExperimentLevel>();
        //SearchExperimentTrial searchExperimentTrial = searchExperimentLevel.currentTrial;
        //searchExperimentTrial.TrialStarted += SearchExperimentTrial_TrialStarted;
        //searchExperimentTrial = searchExperimentLevel.currentTrial;
        //StartCoroutine(WaitForCloningAndCreateDuplicates());
    }

    //private IEnumerator WaitForCloningAndCreateDuplicates()
    //{
    //    yield return StartCoroutine(searchExperimentTrial.WaitForCloningToComplete());
    //    CreateDuplicatesForMiniMap();
    //}

    //private void SearchExperimentTrial_TrialStarted(object sender, EventArgs e)
    //{
    //    CreateDuplicatesForMiniMap();
    //}

    void OnEnable()
    //public void EnableMiniMapInteractor()
    {
        searchExperimentLevel = UnityEngine.Object.FindObjectOfType<SearchExperimentLevel>();
        originalInteractables = FindObjectsOfType<Interactable>().ToList();
    ...
    //private void Update()
    //{
    //    foreach (var kvp in originalToDuplicateMap)
    //    {
    //        GameObject original = kvp.Key;
    //        GameObject duplicate = kvp.Value;

    //        if (searchExperimentLevel.selectStatusDict.ContainsKey(original.GetComponent<Interactable>()))
    //        {
    //            bool isSelected = searchExperimentLevel.selectStatusDict[original.GetComponent<Interactable>()];
    //            UpdateOutline(duplicate, isSelected);
    //        }
    //    }
    //}

    private void UpdateOutline(GameObject duplicate, bool isSelected)
    {
        if (isSelected)
        {
            if (duplicate.GetComponent<Outline>() == null)
            {
                Outline outline = duplicate.AddComponent<Outline>();
            ...
            {
                duplicate.GetComponent<Outline>().OutlineColor = Color.green;
            }
        }
        else
        {
            if (duplicate != null)
            {
                if (duplicate.GetComponent<Outline>() != null)
                {
                    Destroy(duplicate.GetComponent<Outline>());
                }
            }
        }
    }
    ...
    public void UpdateDuplicatesForMiniMap()
    {
        ...
        foreach (var interactable in originalInteractables)
        {
            ...
            originalToDuplicateMap[original] = duplicate;



            if (duplicate.CompareTag("UnknownObject1"))
            {
                duplicate.transform.localScale = new Vector3(0.05f, 0.08f, 0.05f);
            }
            if (duplicate.CompareTag("UnknownObject2"))
            {
                //duplicate.transform.localPosition = new Vector3(300, 100, 100);
                duplicate.transform.localScale = new Vector3(0.05f, 0.05f, 0.05f);
            }
            if (duplicate.CompareTag("UnknownObject3"))
            {
                duplicate.transform.localScale = new Vector3(0.05f, 0.05f, 0.05f);
            }
            if (duplicate.CompareTag("UnknownObject5"))
            {
                duplicate.transform.localScale = new Vector3(0.05f, 0.05f, 0.05f);
            }
            ...
            if (duplicate.CompareTag("cylinder"))
            {
                duplicate.transform.localScale = new Vector3(0.05f, 0.05f, 0.05f);
            }
            if (duplicate.CompareTag("pyramid1"))
            {
                duplicate.transform.localScale = new Vector3(0.05f, 0.05f, 0.05f);
            }
            ...
            if (duplicate.CompareTag("sphere"))
            {
                duplicate.transform.localScale = new Vector3(0.07f, 0.07f, 0.07f);
            }
            if (duplicate.CompareTag("cube"))
            {
                duplicate.transform.localScale = new Vector3(0.06f, 0.06f, 0.06f);
            }
            ...
            if (searchExperimentLevel.selectStatusDict.ContainsKey(interactable))
            {
                //Debug.Log("selectstatusdict contains original object");
                if (searchExperimentLevel.selectStatusDict[interactable] == true)
                {
                    //Debug.Log("selectstatusdict original object is selected");
                    if (duplicate.GetComponent<Outline>() == null)
                    {
                        Outline outline = duplicate.AddComponent<Outline>();
                        outline.OutlineColor = Color.green;
                    ...
                    {
                        duplicate.GetComponent<Outline>().enabled = true;
                        duplicate.GetComponent<Outline>().OutlineColor = Color.green;
                    }
                }
                else
                {
                    //Debug.Log("selectstatusdict original object is not selected");
                    if (duplicate.GetComponent<Outline>() != null)
                    {
                        Destroy(duplicate.GetComponent<Outline>());
                    }
            ...
            originalToDuplicateCir[original] = duplicateOFduplicate;
    ...
    public void CreateDuplicatesForMiniMap()
    {
        ...
        //List<Interactable> originalInteractables = searchExperimentLevel.currentTrial.allObjects.ToList();
        originalInteractables = FindObjectsOfType<Interactable>().ToList();
    ...
    private void Update()
    {
        if (searchExperimentLevel.currentTrial != null)
        {
            if (searchExperimentLevel.currentTrial.temptargetscloned)
            {
                UpdateDuplicatesForMiniMap();
                searchExperimentLevel.currentTrial.temptargetscloned = false;
            }
        }
        ...
        foreach (var kvp in originalToDuplicateMap)
        {
            ...
            //Debug.Log(original.GetComponent<Interactable>());
            //Debug.Log(searchExperimentLevel.selectStatusDict);
            //Debug.Log(searchExperimentLevel.selectStatusDict[original.GetComponent<Interactable>()]);
            if (original != null)
            {
                if (searchExperimentLevel.selectStatusDict.ContainsKey(original.GetComponent<Interactable>()))
                {
                    bool isSelected = searchExperimentLevel.selectStatusDict[original.GetComponent<Interactable>()];
                    UpdateOutline(duplicate, isSelected);
                    //Debug.Log("Adding highlight to " + duplicate.name + "? " + isSelected);
                }
            }
            
        }

        foreach (var kvp in originalToDuplicateCir)
        {
            GameObject original = kvp.Key;
            GameObject duplicateCir = kvp.Value;
            if (original != null)
            {
                if (searchExperimentLevel.selectStatusDict.ContainsKey(original.GetComponent<Interactable>()))
                {
                    bool isSelected = searchExperimentLevel.selectStatusDict[original.GetComponent<Interactable>()];
                    UpdateOutline(duplicateCir, isSelected);
                    //Debug.Log("Adding highlight to " + duplicateCir.name + "? " + isSelected);
                }
            }
        }
    }
